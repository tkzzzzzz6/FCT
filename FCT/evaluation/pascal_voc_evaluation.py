...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        with tempfile.TemporaryDirectory(prefix="disease_eval_") as dirname:
        ...
        # Calculate mean precision and recall for each threshold
        mean_precisions = {}
        mean_recalls = {}

        for iou, prec_list in precisions.items():
            if len(prec_list) > 0:
                mean_precisions[iou] = np.mean(np.concatenate(prec_list), axis=0).tolist()
        for iou, rec_list in recalls.items():
            if len(rec_list) > 0:
                mean_recalls[iou] = np.mean(np.concatenate(rec_list), axis=0).tolist()
        ...
        # Output mean precision and recall
        self._logger.info("Mean Precision:\n" + create_small_table(mean_precisions))
        self._logger.info("Mean Recall:\n" + create_small_table(mean_recalls))
        return ret
